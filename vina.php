...
foreach ($array as $item) {
  ...
  $sql = "INSERT INTO lpf (cuenta,id_tracker,`lat`,`long`,`patente`,`direccion`,`fecha`,`last_update`, `imei`,`connection_status` ) VALUES ('Vina','$id', '$lat', '$lng', '$plate', '$direcc1', '$hoy','$last_u','$imei', '$status'  )";
   ...
   if (mysqli_num_rows($datosduplicados) > 0) {
     ...
     $sql1 = "UPDATE lpf SET `lat`='$lat', `long`='$lng' , `direccion`='$direcc1' ,`last_update`= '$last_u', `fecha`='$hoy', `cuenta`='Vina', `imei`='$imei', `connection_status`='$status' WHERE `id_tracker`='$id' ";
